...
class DataLoaders:
    ...
    def _make_dloader(self, split):
        if self.loader == 'UCR':
            train_data, train_labels, test_data, test_labels = load_UCR(self.dataset, self.data_path)
        ...
        if split == 'train':
            ...
            return DataLoader(train_dataset, batch_size=self.batch_size, shuffle=True), label_num
        elif split == 'test':
            label_num = self._get_label_num(train_labels, test_labels)
            ...
            return DataLoader(test_dataset, batch_size=self.batch_size), label_num
...
def get_dls_cls(params):
    return DataLoaders(loader=params.loader, dataset = params.dset_finetune, data_path=params.dset_path, percentage=params.percentage, 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset_finetune', type=str, default='AllGestureWiimoteX', help='dataset name')
    parser.add_argument('--loader', type=str, default='UCR', help='UCR or UEA')
    parser.add_argument('--dset_path', type=str, default='/home/bigmodel/Decoder_version_1/data/cls_datasets/', help='dataset name')
