...
class PatchCB_mpl(Callback):

    def __init__(self, patch_len_list, stride_list):
        """
        Callback used to perform patching on the batch input data
        Args:
            patch_len:        patch length
    ...
    def set_patch(self):
        ...
        # padding
        if x.shape[1] < max(self.patch_len_list):
            padding = torch.zeros([x.shape[0], self.patch_len - x.shape[1], x.shape[2]]).to(x.device)
            x = torch.cat((x, padding), dim=1)
            
        xb_patch, num_patch = create_patch_mpl(x, self.patch_len_list, self.stride_list)    # xb: [bs x seq_len x n_vars]
        # learner get the transformed input
        self.learner.xb = xb_patch                              # xb_patch: [bs x num_patch x n_vars x patch_len]
        # self.learner.yb = xb_patch                              # xb_patch: [bs x num_patch x n_vars x patch_len]
...
def create_patch_mpl(xb, patch_len_list, stride_list):
    """
    xb: [bs x seq_len x n_vars]
    """
    seq_len = xb.shape[1]
    ...
    for i in range(len(patch_len_list)):
        num_patch = (max(seq_len, patch_len_list[i])-patch_len_list[i]) // stride_list[i] + 1
        tgt_len = patch_len_list[i]  + stride_list[i]*(num_patch-1)
        s_begin = seq_len - tgt_len
        
        xb_i = xb[:, s_begin:, :]                                                    # xb: [bs x tgt_len x nvars]
        xb_i = xb_i.unfold(dimension=1, size=patch_len_list[i], step=stride_list[i])                 # xb: [bs x num_patch x n_vars x patch_len]
            
        xb_total.append(xb_i)
        num_patch_total.append(num_patch)

    return xb_total, num_patch_total
