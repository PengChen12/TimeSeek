...
class TrackTrainingCB(Callback):
    ...
    def initialize_recorder(self):
        recorder = {'epoch': [],  'train_loss': [],  'train_mse': [], 'train_pred_loss': [], 'train_contrastive_loss': []} 
        if self.valid_loss: 
            recorder['valid_loss'] = []
    ...
    def initialize_batch_recorder(self, with_metrics):
        batch_recorder = {'n_samples': [], 'batch_losses': [], 'batch_mses': [], 'batch_pred_losses': [], 'batch_contrastive_losses': [], 'with_metrics': with_metrics}                                                         
    ...
    def after_epoch_train(self):         
        ...
        self.recorder['train_contrastive_loss'].append(values['contrastive_loss'])
    ...
    def after_epoch_valid(self):             
        ...
        self.recorder['valid_contrastive_loss'].append(values['contrastive_loss'])
        # save metrics after one epoch         
    ...
    def accumulate(self ):
        ...
        contrastive_loss = self.contrastive_loss*bs if self.mean_reduction_ else self.contrastive_loss      
        ...
        self.batch_recorder['batch_contrastive_losses'].append(contrastive_loss)
        ...
        if self.iter % 500 == 0:
            ...
            cur_contrastive_loss = sum(self.batch_recorder['batch_contrastive_losses'])/n  # averaging
            print(f"{self.iter} iter loss: {cur_loss:.4f}  iter mse: {cur_mse:.4f} iter pred_loss: {cur_pred_loss:.4f} iter contrastive_loss: {cur_contrastive_loss:.4f}")
    ...
    def compute_scores(self):
        ...
        values['contrastive_loss'] = sum(self.batch_recorder['batch_contrastive_losses'])/n  # averaging
