...
class TrackTrainingCB(Callback):
    ...
    def initialize_recorder(self):
        recorder = {'epoch': [],  'train_loss': [], 'train_accuracy': []} 
        if self.valid_loss: 
            recorder['valid_loss'] = []
            recorder['valid_accuracy'] = []
    ...
    def initialize_batch_recorder(self, with_metrics):
        batch_recorder = {'n_samples': [], 'batch_losses': [], 'batch_accuracies': [], 'with_metrics': with_metrics}                                                         
    ...
    def after_epoch_train(self):         
        ...
        self.recorder['train_accuracy'].append( values['accuracy'] )
    ...
    def after_epoch_valid(self):             
        ...
        self.recorder['valid_accuracy'].append( values['accuracy'] )
        # save metrics after one epoch         
    ...
    def accumulate(self ):
        ...
        accuracy = self.accuracy*bs if self.mean_reduction_ else self.accuracy    
        # mse = self.mse.detach()*bs if self.mean_reduction_ else self.loss.detach()            
        self.batch_recorder['batch_losses'].append(loss)
        self.batch_recorder['batch_accuracies'].append(accuracy)
        ...
        if self.iter % 500 == 0:
            ...
            cur_accuracy = sum(self.batch_recorder['batch_accuracies'])/n  # averaging
            print(f"{self.iter} iter loss: {cur_loss:.4f} iter accuracy: {cur_accuracy:.4f}")
    ...
    def compute_scores(self):
        ...
        values['accuracy'] = sum(self.batch_recorder['batch_accuracies'])/n  # averaging
