...
class PatchTST(nn.Module):
    ...
    def __init__(self, c_in:int, target_dim:int, patch_len:int, stride:int, num_patch:int, mask_mode:str = 'patch',mask_nums:int = 3,
                 ...
                 head_type = "prediction",img_size = 64,
        ...
        self.resize = Resize([self.img_size, self.img_size], interpolation=Image.BILINEAR)
        ...
        # Position Embedding
        # self.pos = positional_encoding(pe, learn_pe, 3 + num_patch, d_model)
    ...
    def ts_resize(self, x):
        ...
        pic.save('/home/Decoder_version_2/visualization/C_top_4.jpg')
    ...
    def forward(self, z):                             
        ...
        # x_2d
        z_2d = self.ts_resize(z).reshape(3*bs*n_vars, 3, self.img_size, self.img_size)
        # z_2d = self.ts_resize(z).reshape(bs*n_vars, 3, self.img_size, self.img_size)
        img_tokens = self.image_encoder(z_2d).reshape(-1, bs, n_vars, self.d_model).permute(1,2,0,3)
        img_tokens = F.max_pool1d(
            img_tokens.reshape(bs*n_vars*self.d_model, 3),
            kernel_size = 3,
        ).reshape(bs, n_vars, self.d_model) # [bs x n_vars x d_model]

        img_tokens = img_tokens.unsqueeze(2).reshape(bs, n_vars, -1, self.d_model)
        # tokenizer
        ...
        # encoder 
        # z = torch.reshape(z, (-1, 3 + self.num_patch, self.d_model))
        ...
        # z = torch.reshape(z, (-1, n_vars, 3 + self.num_patch, self.d_model))
        
        # predict
        z = self.head(z.transpose(2,3))   
        # z = self.head(img_tokens.transpose(2,3))
...
class ClassificationHead(nn.Module):
    def __init__(self, n_vars, d_model, n_classes, head_dropout):
        super().__init__()
        self.flatten = nn.Flatten(start_dim=1)
        # self.linear = ProjectionHead(3*n_vars*d_model, n_classes, 128)
    ...
    def forward(self, x):
        ...
        x = x[:,:,:,0]
        # x = x[:,:,:,0:3] # only consider the First item in the sequence, x: bs x nvars x d_model
        # x_ts = x[:,:,:,3:].reshape(bs*n_vars, d_model, -1)
        # x_ts = torch.max_pool1d(x_ts, num_patch-3)
        # x_ts = x_ts.reshape(bs, n_vars, d_model, 1)
        # x = torch.cat([x_img,x_ts],dim=-1)
...
#     frequency_list[:,0] = 0
#     _, top_list = torch.topk(frequency_list, k, dim=1)
#     top_list = top_list.detach().cpu().numpy()
#     period = x.shape[1] // top_list
#     return period