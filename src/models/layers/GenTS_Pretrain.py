...
class PatchTST(nn.Module):
    ...
    def __init__(self, c_in: int, target_dim: int, patch_len: int, stride: int, target_patch_len: int, num_patch: int,
        ...
        # Encoder
        ##这里可能不需要输入长度，直接去除掉
        self.encoder_layer_scale2 = MOE_layers(self.num_patch / 4, d_model, n_heads, d_ff, e_layers, num_slots=20)  ##最粗尺度
        self.encoder_layer_scale1 = MOE_layers(self.num_patch / 2, d_model, n_heads, d_ff, e_layers, num_slots=20)  ##中间尺度
        self.encoder_layer_scale0 = MOE_layers(self.num_patch, d_model, n_heads, d_ff, e_layers, num_slots=20)  ##最细尺度
