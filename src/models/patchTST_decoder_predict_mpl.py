...
class PatchTST(nn.Module):
    ...
    def __init__(self, c_in:int, target_dim:int, patch_len:int, stride:int, num_patch:int, mask_mode:str = 'patch',mask_nums:int = 3,
        ...
        # self.mpl_weight = nn.Parameter(torch.randn(3,1,1,c_in), requires_grad=True)
        self.mpl_weight = nn.Parameter(torch.ones([3,1,1,c_in]), requires_grad=True)
        ...
        # Position Embedding
        self.pos = positional_encoding(pe, learn_pe, 1 + num_patch*patch_len + self.out_patch_num, d_model)
    ...
    def decoder_predict(self, bs, n_vars, dec_cross, out_patch_num):
        """
        dec_cross: tensor [bs x  n_vars x num_patch x d_model]
        """
        bs, n_vars, num_patch, d_model = dec_cross.shape
        
        dec_in = self.decoder_embedding.expand(bs, self.n_vars, out_patch_num, -1)
        ...
        dec_in = dec_in + self.pos[1+num_patch:num_patch + out_patch_num+1,:]
    ...
    def forward(self, z_list):                             
        ...
        """   
        patch_len_num = len(z_list)

        for i in range(patch_len_num):
            z = z_list[i]
            ...
            z = self.drop_out(z + self.pos[:1 + num_patch, :])
            ...
            # decoder
            out_patch_num = math.ceil(self.target_dim / patch_len)
            z = self.decoder_predict(bs, n_vars, z[:,:,:,:], out_patch_num)
            
            # predict
            z = self.head(z, patch_len)    
            z = z[:,:self.target_dim, :]  

            if i == 0:
                ouput_list = z.unsqueeze(0)
            else:
                ouput_list = torch.concat([ouput_list,z.unsqueeze(0)], dim=0)
        
        # mpl_weight = self.sm(self.mpl_weight)
        # mpl_weight = F.gumbel_softmax(self.mpl_weight, tau=1, hard=True, dim=0)
        # mpl_weight = self.mpl_weight/self.mpl_weight.sum()
        # mpl_weight = mpl_weight.view(3,1,1,self.mpl_weight)
        # print(mpl_weight)
        # ouput = torch.sum(ouput_list*mpl_weight, dim=0)
        ouput = torch.mean(ouput_list, dim=0)
