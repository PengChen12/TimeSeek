...
class PatchTST(nn.Module):
    ...
    def forward(self, z):                             
        ...
        self.sep_tokens = self.sep_embedding.expand(bs, n_vars, -1, -1)
        ...
        # # mask_token
        # dec_in = z[:,:,-1,:].unsqueeze(2).expand(-1,-1,self.out_patch_num,-1)
        # weights = self.get_dynamic_weights(self.out_patch_num).to(dec_in.device)
        # dec_in = dec_in * weights.unsqueeze(0).unsqueeze(0).unsqueeze(-1)
        # z = torch.cat((z,dec_in), dim = 2)
        ...
        # predict
        # z = self.head(z[:,:,-self.out_patch_num:,:].permute(0,1,3,2))  
        z = self.head(z[:,:,:,:])    
        z = z[:,:self.target_dim, :]  
