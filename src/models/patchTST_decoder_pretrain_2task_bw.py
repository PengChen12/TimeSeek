...
class PatchTST(nn.Module):
    ...
    def forward(self, z):                             
        ...
        img_tokens = img_tokens.unsqueeze(0).expand((mask_nums + 1), -1, -1, -1, -1).reshape(bs*(mask_nums + 1), n_vars, -1, self.d_model)
        ...
        z = torch.cat((img_tokens, z), dim=2)  # [bs*(1+mask_nums) x n_vars x (3 + num_patch) x d_model]
        z = self.drop_out(z + self.pos[:3 + self.num_patch, :])

        # encoder 
        z = torch.reshape(z, (-1, 3 + self.num_patch, self.d_model)) # [bs*(1+mask_nums)*n_vars x num_patch x d_model]
        z = self.encoder(z)
        z = torch.reshape(z, (-1, n_vars, 3 + self.num_patch, self.d_model)) # [bs*(1+mask_nums), n_vars x num_patch x d_model]
        z_masked = z[bs:,:,3:,:] # [bs*mask_nums, n_vars x num_patch x d_model]
        z_original = z[0:bs,:,3:,:] # [bs, n_vars x num_patch x d_model]

        # aggregation
        if self.mask_mode == "multi" or self.mask_mode == "freq_multi":
            z_masked = z_masked.reshape(self.mask_nums, -1, n_vars, num_patch, self.d_model,).mean(0).transpose(2,3)     # z_masked: [bs x nvars x d_model x num_patch]    
        ...
        # recontruction
        z_reconstruct = self.head(z_masked[:,:,:,:])
