...
class ImageEncoder(nn.Module):
    def __init__(self, out_dim):
        ...
        # 3.修改输出的类别为10
        ResNet.fc = nn.Linear(fc_features,out_dim)
        self.encoder = ResNet
...
class PatchTST(nn.Module):
    ...
    def __init__(self, c_in:int, target_dim:int, patch_len:int, stride:int, num_patch:int, mask_mode:str = 'patch',mask_nums:int = 3,
        ...
        #                            pre_norm=pre_norm, activation=act, res_attention=res_attention, n_layers=1, 
    ...
    def forward(self, z, img):                             
        ...
        # img_tokens && corr
        bs, n_vars, C, H, W = img.shape
        img_tokens = self.img_encoder(img.reshape(bs*n_vars, C , H, W)).reshape(bs,n_vars,self.d_model)
        # img_tokens = self.corr_encoder(img_tokens).reshape(bs,n_vars,1,self.d_model)
        # img_tokens = img_tokens.unsqueeze(0).repeat((1+mask_nums), 1, 1, 1, 1).reshape(bs*(1+mask_nums), n_vars, 1, self.d_model) # [bs*(1+mask_nums) x n_vars x num_patch x d_model]

        # tokenizer   
        cls_tokens = self.cls_embedding.expand(bs*(1+mask_nums), n_vars, -1, -1)
        ...
        if self.mask_mode == "multi" or self.mask_mode == "freq_multi":
            z_masked = z.reshape(1 + self.mask_nums, -1, n_vars, num_patch + 1, self.d_model,).mean(0).transpose(2,3)     # z_masked: [bs x nvars x d_model x num_patch]    
        ...
        return z_reconstruct, z_predict, self.projection_img(img_tokens), self.projection_cls(cls_tokens_original)
