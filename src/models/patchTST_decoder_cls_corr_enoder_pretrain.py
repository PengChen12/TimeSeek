...
class PatchTST(nn.Module):
    ...
    def __init__(self, c_in:int, target_dim:int, patch_len:int, stride:int, num_patch:int, mask_mode:str = 'patch',mask_nums:int = 3,
        ...
        # Position Embedding
        self.pos = positional_encoding(pe, learn_pe, num_patch+1, d_model)
        self.drop_out = nn.Dropout(dropout)

        self.corr_module = Corr_module(n_vars=c_in, patch_len=patch_len, num_patch=num_patch, n_layers=3, d_model=d_model, 
                                       n_heads=n_heads, d_ff=d_ff, codebook_size=128, norm=norm, attn_dropout=attn_dropout, dropout=dropout,
    ...
    def forward(self, z):                             
        ...
        # tokenizer
        corr_tokens, _ = self.corr_module(z)  # [bs x n_vars x d_model]
        ...
        # z = z + self.pos
        z = torch.cat((corr_tokens, z), dim=2)  # [bs x n_vars x (n_vars + num_patch) x d_model]
        z = self.drop_out(z + self.pos[:1+ self.num_patch, :])

        # encoder 
        z = torch.reshape(z, (-1, self.num_patch+1, self.d_model)) # [bs*n_vars x num_patch x d_model]
        z = self.encoder(z)
        z = torch.reshape(z, (-1, n_vars, self.num_patch+1, self.d_model)) # [bs, n_vars x num_patch x d_model]
