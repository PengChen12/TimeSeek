...
class resample_embedding(nn.Module):

  def __init__(self, patch_len: int = 32, d_model: int = 768):
    super(resample_embedding, self).__init__()

    # Basic
    self.patch_len = patch_len

    # 初始化patch embedding的线性层
    self.linear = nn.Linear(patch_len, d_model)

  def forward(self, x):

    _, _, _, patch_len = x.shape  # 获取序列的形状，n是批次大小，seq_len是序列长度

    # 如果计算出的patch长度与默认的patch长度不同，则重新采样内核
    if patch_len != self.patch_len:
      self.linear.weight.data = resample_patchemb(old=self.linear.weight.data, new_patch_len=patch_len)
      self.linear.in_features = patch_len  # 更新线性层的输入特征数
...
class PatchTST(nn.Module):
    ...
    def __init__(self, c_in:int, target_dim:int, patch_len:int, stride:int, num_patch:int, mask_mode:str = 'patch',mask_nums:int = 3,
        ...
        # Embedding
        # self.embedding = resample_embedding(patch_len=self.target_patch_len, d_model=d_model)
    ...
    def forward(self, z):                             
        ...
        if patch_len != self.target_patch_len:
            self.embedding.weight.data = resample_patchemb(old=self.embedding.weight.data, new_patch_len=patch_len)
            self.embedding.in_features = patch_len  # 更新线性层的输入特征数
...
class decoder_PredictHead(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.patch_len != self.target_patch_len:
            self.linear.weight.data = resample_patchemb(old=self.linear.weight.data.T, new_patch_len=self.patch_len).T
            self.linear.bias.data = resample_patchemb(old=self.linear.bias.data.unsqueeze(0), new_patch_len=self.patch_len).squeeze(0)
            self.linear.out_features = self.patch_len
