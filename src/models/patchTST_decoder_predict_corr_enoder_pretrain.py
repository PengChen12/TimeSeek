...
class PatchTST(nn.Module):
    ...
    def __init__(self, c_in:int, target_dim:int, patch_len:int, stride:int, num_patch:int, mask_mode:str = 'patch',mask_nums:int = 3,
        ...
        # Position Embedding
        self.pos = positional_encoding(pe, learn_pe, c_in + num_patch + self.out_patch_num, d_model)
    ...
    def decoder_predict(self, bs, n_vars, dec_cross):
        ...
        # dec_in = dec_cross[:,:,0,:].unsqueeze(2).expand(-1,-1,self.out_patch_num,-1)
        dec_cross = dec_cross[:,:,1:,:]
    ...
    def forward(self, z):                             
        ...
        corr_tokens = self.corr_module(z)  # [bs x n_vars x d_model]
        corr_tokens = corr_tokens.unsqueeze(2) # [bs x n_vars x 1 x d_model]

        # multi_corr_tokens
        # corr_tokens = corr_tokens.permute(0,2,1,3).expand(-1, n_vars, -1, -1)  # [bs x n_vars x n_vars x d_model]

        
        # tokenizer
        # cls_tokens = self.cls_embedding.expand(bs, n_vars, -1, -1)
        z = self.embedding(z).permute(0,2,1,3).contiguous() # [bs x n_vars x num_patch x d_model]

        # corr
        # corr_tokens = torch.mean(z, dim=2) # [bs x n_vars x d_model]
        # corr_tokens = z.reshape(bs*n_vars, num_patch, self.d_model).permute(0,2,1) #[bs*n_vars x d_model x num_patch]
        # corr_tokens = torch.max_pool1d(corr_tokens, corr_tokens.shape[2]).squeeze(-1).reshape(bs, n_vars, self.d_model)
        # print(corr_tokens.shape)

        # cat token
        # z = torch.cat((cls_tokens, z), dim=2)  # [bs x n_vars x (1 + num_patch) x d_model]
        z = torch.cat((corr_tokens, z), dim=2)  # [bs x n_vars x (1 + num_patch) x d_model]
        z = self.drop_out(z + self.pos[:1 + self.num_patch, :])
        # z = self.drop_out(z + self.pos[:self.num_patch, :])

        # encoder 
        z = torch.reshape(z, (-1, 1 + self.num_patch, self.d_model)) # [bs*n_vars x num_patch x d_model]
        # z = torch.reshape(z, (-1, self.num_patch, self.d_model)) # [bs*n_vars x num_patch x d_model]
        z = self.encoder(z)
        z = torch.reshape(z, (-1, n_vars, 1 + self.num_patch, self.d_model)) # [bs, n_vars x num_patch x d_model]
        # z = torch.reshape(z, (-1, n_vars, self.num_patch, self.d_model)) # [bs, n_vars x num_patch x d_model]
...
class Corr_module(nn.Module):
    ...
    def forward(self, z):                             
        ...
        # z=self.maxpool(z.permute(0,2,3,1).reshape(-1,self.d_model,num_patch)).reshape(bs,n_vars,self.d_model)
        z = z[:,-1,:,:]  # [bs x  n_vars x d_model]
        # z_q=z
        # mask
        # z=self.channel_mask(input=z, mask_ratio=0.4)  # [bs x n_vars x d_model]
        # encoder 
        z_d = self.corr_encoder(z)  # [bs x n_vars x d_model]
