...
from .callback.tracking_corr_encoder_pretrain import * 
...
class Learner(GetAttr):
    ...
    def train_step(self, batch):
        ...
        if self.codebook_size!=0:
            # forward
            z_d, z_q, z, z_q_g= self.model_forward()
            # compute loss
            ...
            loss_embedding = loss_func1(z.detach(), z_q_g)
            loss_commitment = loss_func1(z, z_q_g.detach())
            loss_contrastive = temporal_contrastive_loss(z_d,z_q)
            self.contrastive_loss= loss_contrastive
            loss = loss_contrastive + loss_embedding +  loss_commitment
        else:
            # forward
            z_d, z_q= self.model_forward()
            loss = temporal_contrastive_loss(z_d,z_q)
