...
class Learner(GetAttr):
    ...
    def train_step(self, batch):
        ...
        # forward
        pred, reconstruct, reg = self.model_forward()
        # compute loss
        real_reconstruct, real_pred=self.yb
        loss_reconstruct = self.loss_func(real_reconstruct, reconstruct)
        loss_func_pred=torch.nn.L1Loss(reduction='mean')
        loss_pred = loss_func_pred(pred, real_pred)

        loss_func1 = torch.nn.MSELoss(reduction='mean')
        self.mse = loss_func1(pred, real_pred)

        loss = loss_reconstruct + loss_pred + reg
        return pred, loss

    def model_forward(self):
        self('before_forward')
        self.pred, reconstruction, reg = self.model(self.xb)
        self('after_forward')
        return self.pred, reconstruction, reg
    ...
    def test_cka(self, dl, weight_path=None):
        if dl is None: return
        else: self.dl = dl
        if weight_path is not None: self.load(weight_path)
        cka_sim = []
        ...
        with torch.no_grad():
            for i, self.batch in enumerate(self.dl):
                self.xb, self.yb = self.batch
                self.xb = self.xb.to('cuda')
                cka = self.model_forward()
                cka_sim.append(cka.detach().cpu().numpy())
        

        print(sum(cka_sim)/len(cka_sim))
...
def visual(input ,true, preds=None, path='./pic/test.pdf'):
    ...
    plt.figure()    
    plt.plot(ground_truth[-gt_len:], label='GroundTruth', linewidth=2,c='royalblue')
    plt.plot(np.arange(pred_len) + input_len, prediction, label='Prediction', linewidth=2,  c='darkorange')
