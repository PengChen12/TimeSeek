...
class Learner(GetAttr):
    ...
    def train_step(self, batch):
        ...
        if self.n_embedding!=0:
            ...
            # forward
            reconstruct, pred, img_tokens, cls_tokens = self.model_forward()
            real_reconstruct, real_pred = self.yb
            # compute loss
            loss_reconstruct = self.loss_func(reconstruct, real_reconstruct)
            # loss_contrastive = img_loss(img_tokens, cls_tokens)
            loss_func1=torch.nn.MSELoss(reduction='mean')
            loss_pred=loss_func1(pred, real_pred)
    ...
    def model_forward(self):
        ...
        if self.n_embedding!=0:
            ...
            self.reconstruct, self.pred, self.img_tokens, self.cls_tokens = self.model(self.xb)
            self('after_forward')
            return self.reconstruct, self.pred, self.img_tokens, self.cls_tokens
