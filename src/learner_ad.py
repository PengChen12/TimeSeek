...
class Learner(GetAttr):
    ...
    def train_step(self, batch):
        ...
        # c = self.xb.shape[2]
        # sigma = 1
        # random_array = np.random.permutation(c)
        # lambdas = torch.from_numpy(np.random.normal(loc=0, scale=sigma, size=(c,))).float().to(self.xb.device)
        # self.xb = self.xb + lambdas * self.xb[:, :, random_array]
        # self.yb = self.yb + lambdas * self.yb[:, :, random_array]
