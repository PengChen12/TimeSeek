...
from .callback.tracking_3task_img import * 
...
class Learner(GetAttr):
    ...
    def train_step(self, batch):
        ...
        if self.n_embedding!=0:
            ...
            real_reconstruct,real_pred = self.yb
            # compute loss
            loss_contrastive = contrastive_loss(img_tokens, cls_tokens)
            loss_reconstruct = self.loss_func(reconstruct, real_reconstruct)
            loss_func1=torch.nn.MSELoss(reduction='mean')
            loss_pred=loss_func1(pred,real_pred)
            ...
            loss = loss_reconstruct + loss_pred + loss_contrastive
        return pred, loss
    ...
    def model_forward(self):
        ...
        if self.n_embedding!=0:
            ...
            self.reconstruct,self.pred, self.img_tokens, self.cls_tokens  = self.model(self.xb, self.img)
            self('after_forward')
...
def contrastive_loss(z1, z2, scale=1):
    B, C = z1.size(0), z1.size(1)

    if C == 1:
        return z1.new_tensor(0.)

    z1_f = z1 / z1.norm(dim=2, keepdim=True)
    z2_f = z2 / z2.norm(dim=2, keepdim=True)

    z = torch.cat([z1_f, z2_f], dim=1)  # B x 2C x C
    sim = torch.matmul(z, z.transpose(1, 2))    # B x 2C x 2C
    logits = torch.tril(sim, diagonal=-1)[:, :, :-1]    # B x 2C x (2C-1)
