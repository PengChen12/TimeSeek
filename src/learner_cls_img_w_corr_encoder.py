...
from .callback.tracking_cls_img import * 
...
class Learner(GetAttr):
    ...
    def train_step(self, batch):
        # get the inputs
        self.xb, self.yb , self.img= batch
        # forward
        ...
        y_pred = pred.detach().argmax(dim=1)  # (n,)
        y_target = self.yb.detach().argmax(dim=1) # (n,)
    ...
    def model_forward(self):
        self('before_forward')
        self.pred = self.model(self.xb, self.img)
        self('after_forward')
        return self.pred
    ...
    def test_ad(self, dl, weight_path=None, metrics=None, folder_path:str=None):
        if dl is None: return
        else: self.dl = dl.test
        ...
        with torch.no_grad():
            for i, self.batch in enumerate(self.dl):
                ...
                batch_x , batch_y = self.batch
                self.xb = self.xb.to('cuda')
                batch_x = batch_x.to('cuda')
                reconstruction = self.model_forward()
                score = anomaly_criterion(batch_x, reconstruction)
        ...
        evaluate(gt, test_scores, metrics=metrics, save_path=folder_path, verbose=True)
    
    def test_ad_spot(self, dl, weight_path=None, metrics=None, folder_path:str=None, qs = None):
        anomaly_criterion = nn.MSELoss(reduce=False)
        self.model.eval()
        # cal anomaly_socres
        test_scores = []
        ...
        test_labels = []
        with torch.no_grad():
            for i, (self.batch) in enumerate(dl.init):
                self.xb, self.yb ,self.img= self.batch
                batch_x , batch_y , img= self.batch
                self.xb = self.xb.to('cuda')
                self.img = self.img.to('cuda')
                batch_x = batch_x.to('cuda')
                img = img.to('cuda')
            ...
            for i, self.batch in enumerate(dl.test):
                self.xb, self.yb ,self.img= self.batch
                batch_x , batch_y , img= self.batch
                self.xb = self.xb.to('cuda')
                self.img = self.img.to('cuda')
        ...
        evaluate_SPOT(gt, init_scores, test_scores, metrics=metrics, save_path=folder_path, qs=qs, verbose=True)

    def test_cls(self, dl, weight_path=None, result_path=None, dataset=None):
        ...
        self.model.eval()
        with torch.no_grad():
            for index, self.batch in enumerate(dl):
                self.xb, self.yb, self.img= self.batch
                batch_x , batch_y , img= self.batch
                self.xb = self.xb.to('cuda')
                self.img = self.img.to('cuda')
                ...
                y_target = batch_y.argmax(dim=1) # (n,)
                total += y_pred.shape[0]
                correct += (y_pred == y_target).sum().item()           
        ...
        pd.DataFrame([res]).to_csv(result_path, index=False, mode='a', header=False)
    ...
    def unfreeze(self):
        ...
        # for param in get_model(self.model).corr_module.parameters(): param.requires_grad = False
        # for param in get_model(self.model).img_encoder.parameters(): param.requires_grad = False
        # for param in get_model(self.model).img_encoder.encoder.fc.parameters(): param.requires_grad = True
