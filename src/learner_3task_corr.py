...
class Learner(GetAttr):
    ...
    def train_step(self, batch):
        ...
        if self.n_embedding!=0:
            # forward
            reconstruct, pred, corr_proj, corr_q_proj, corr_q_g, corr_ori = self.model_forward()
            # compute loss
            ...
            loss_embedding = loss_func1(corr_ori.detach(), corr_q_g)
            loss_commitment = loss_func1(corr_ori, corr_q_g.detach())
            loss_corr_reconstruct = loss_func1(corr_proj, corr_q_proj)
            loss = loss_reconstruct + loss_pred + loss_embedding +  loss_commitment +loss_corr_reconstruct
        else:
            # forward
    ...
    def model_forward(self):
        self('before_forward')
        if self.n_embedding!=0:
            self.reconstruct, self.pred, corr_proj, corr_q_proj, corr_q_g, corr_ori= self.model(self.xb)
            self('after_forward')
            return self.reconstruct,self.pred, corr_proj, corr_q_proj, corr_q_g, corr_ori
        else:
